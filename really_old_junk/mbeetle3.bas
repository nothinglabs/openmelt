...
Exponential = 1.18                                          'greater number = more exponential steering
...
If Eprom_check = 555 Then                                   'load config data from eprom if eprom_check was set to 555
   ...
   Led_adjust_backup = Led_adjust_store
...
If Configmode = 1 Then Throttle_percent = 75                'ignore throttle if in configmode
   ...
   'set heading beacon size and location
    If No_led = 0 Then                                      'skip LED recalc (if we changed straffing)
...
'     Braking_length = Delaytime_long * 25         'original
      Braking_length = Delaytime_long * 12                  'for flight testing
   ...
   'causing LED "streaks?"
   If Led_on > Led_off Then Led_is_on_now = 1               'make sure LED is in right state - helps reduce blink when switching to strafing
'   If Led_on < Led_off Then Led_is_on_now = 0               'make sure LED is in right state - helps reduce blink when switching to strafing
...
'      Straff_blink_check                                    'nasty code to make sure we don't blink due to straff adjustment
