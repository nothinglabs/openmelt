...
'portc.0 - Throttle  (87 = low, 115 = middle, 148 = high)
'portc.1 - Left / Right (425 = left, 469 = center, 541 = right)
'portc.2 - Forward / Back (145 = forward, 114 = center, 83 = back)


'adc.4 accelerometer
'adc.6 battery voltage
'portd.3 - heading indicator LED
'portd.1 - motor 1
'portd.2 - motor 2
...
Enable Interrupts
Enable Pcint1
On Pcint1 Rc_change                                         'call RC_change anytime RC pins go up or down
Pcmsk1 = &B00000111                                         'sets mask so that only RC pins trigger interrupt
...
Const Min_rpm = 400                                         'minimum RPM for translation / throttling to kick in

Const Radius = 1.8                                          'effective radius of circle for accel (centimeters) - seems to be off sometimes...
...
'If Accel_raw_data > 505 Then Portd.3 = 1
'Waitms 50
'Portd.3 = 0
'Waitms 50
'Wend
...
'for bootup - allows window to program / erase chip if needed
Disable Interrupts
...
'if throttle is lower than 90 bot stays powered down
   While Throttle < 90 Or Throttle > 200 Or Shutdown = 1
      ...
      'if stick is pulled back and have signal (throttle over 50) - flash out highest RPM
      If Forwardback < 105 And Throttle > 50 Then
...
'sit there and flash LED if have signal (throttle over 50) - otherwise LED is just on
...
'   Voltage = Getadc(0)                                      'get voltage data (word)
...
'for voltage-based RPM
'   Rpm = Throttle - 10
'   Rpm = Rpm * 14

'   Current_volt = Getadc(0)                                 'not using voltage compensation currently...
...
'   If Throttle > 146 Then Full_power_spin = 1
   ...
   If Full_power_spin = 1 Then
      ...
      For X = 1 To Delaytime_long
         ...
         If Led_is_on_now = 0 Then Portd.3 = 0

         If Led_is_on_now = 1 Then
            If In_tracking_adjust = 1 Or Turbo = 1 Then Toggle Portd.3 Else Portd.3 = 1
         End If
   ...
   Led_ref = 0
      ...
      For X = 1 To Delaytime_long                           'each loop is 1ms (delaytime is length of 180 degrees of cycle)
         ...
         If Low_voltage = 1 And Alternate_motor_cycle = 1 Then Portd.3 = 1
...
'Waitms 50
   ...
   If Pinc.2 <> Forwardback_hilow Then

      If Pinc.2 = 0 Then                                    'did the pin go low? - then set timer value as value for this channel...
         Forwardback = Timer0
         Forwardback = Forwardback * 25
         Forwardback = Forwardback / 10
      End If


      If Pinc.2 = 1 Then                                    'did the pin go high? - then reset timer...
         Timer0 = 0
      End If
   ...
   If Pinc.1 <> Leftright_hilow Then

      If Pinc.1 = 0 Then

         If Timer1 < 260 Then                               'only set if within bounds
            If Timer1 > 120 Then
               Leftright = Timer1
   ...
   If Pinc.0 <> Throttle_hilow Then

      If Pinc.0 = 0 Then

         If Timer2 < 80 Then                                'only set if within bounds
            If Timer2 > 20 Then
               Throttle = Timer2
   ...
   Throttle_hilow = Pinc.0                                  'make note of all pin states for reference next time interrupt is triggered...
   Leftright_hilow = Pinc.1
   Forwardback_hilow = Pinc.2
