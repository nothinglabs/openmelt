...
Const Radius = 1.02                                         'effective radius of circle for accel (centimeters) - seems to be off sometimes...
Const G_per_adc_increment = .5                              '10mv / g, 5mv per single increment up to 1024
Const Base_accel = 489                                      'ADC value for accel with no motion
...
Portd.7 = 1                                                 'turn on power for accel
...
'if throttle is lower than 90 bot stays powered down
   ...
   Voltage = Getadc(6)                                      'get voltage data (word)
      ...
      If Voltage < 35 Then Low_voltage = 1 Else Low_voltage = 0       '35 is about 6.7v
...
'data is written into eprom next time the robot spins down
   ...
   If Throttle > 146 Then Turbo = 1                         'just for flashing effect
...
'   If Throttle > 146 Then Full_power_spin = 1
   ...
   If Full_power_spin = 1 Then
      ...
      For X = 1 To Delaytime_long
         ...
         If Led_is_on_now = 1 Then
            If In_tracking_adjust = 1 Or Turbo = 1 Then Toggle Portd.5 Else Portd.5 = 1
         End If
   ...
   Led_ref = 0
      ...
      For X = 1 To Delaytime_long                           'each loop is 1ms (delaytime is length of 180 degrees of cycle)
         ...
         If Low_voltage = 1 And Alternate_motor_cycle = 1 Then Portd.5 = 1
