...
Portd.7 = 1                                                 'turn on signal LED before timers so it comes on immediately
...
Const Min_rpm = 0                                           'lower RPM for flight testing
'Const Min_rpm = 700                                         'minimum RPM for translation / throttling to kick in

'for hover
Const Radius = .9                                           'effective radius of circle for accel (centimeters) - seems to be off sometimes...
'Const Radius = 2.2915                                       'effective radius of circle for accel (centimeters) - seems to be off sometimes...
Const G_per_adc_increment = .042                            '174mv / g, 5mv per single increment up to 1024
Const Base_accel = 510                                      'ADC value for accel with no motion
...
Max_allowed_rpm = 1450                                      'max speed allowed
...
'Dim Accel_raw_data2 As Word

'While A = 1
'   Accel_raw_data2 = Accel_raw_data
...
'data is written into eprom next time the robot spins down
   ...
   'for hover testing
    Power_kill_offset = Throttle_percent * 95               'set time in each cycle to cut power (throttling)
...
'   If Throttle > 146 Then Full_power_spin = 1
   ...
   If Full_power_spin = 1 Then
      ...
      For X = 1 To Delaytime_long
         ...
         If Led_is_on_now = 0 Then Portd.7 = 0

         If Led_is_on_now = 1 Then
            If In_tracking_adjust = 1 Or Turbo = 1 Then Toggle Portd.7 Else Portd.7 = 1
         End If
...
'         If X > Power_kill_offset Then Motors_off           'if throttle is less that 100% - kill throttle at appropriate time
         ...
         If Low_voltage = 1 And Alternate_motor_cycle = 1 Then Portd.7 = 1
