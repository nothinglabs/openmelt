...
'   If Accel_raw_data > 522 Then Toggle portd.7
'   Waitms 40
'Wend
   ...
   Led_ref = 0
      ...
      For X = 1 To Delaytime_long                           'each loop is 1ms (delaytime is length of 180 degrees of cycle)
         ...
         If Led_is_on_now = 1 Then
               If Flashy_led = 1 Then Toggle portd.7 Else portd.7 = 1
         End If
...
Sub Turn_on_tail
         If Tail_start <= Tail_end Then
            If Led_ref => Tail_start And Led_ref <= Tail_end Then portd.7 = 1
         Else
            If Led_ref => Tail_start Then portd.7 = 1
            If Led_ref <= Tail_end Then portd.7 = 1
         End If
End Sub
