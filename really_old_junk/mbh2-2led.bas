...
'this is a two-wheeled version with full h-bridges - braking testing / etc.


'using in melty b for 4/10/09
...
'portd.5 - heading indicator LED (on motor controller m1 / m2)


'portd.3 - motor 1a
'portc.1 - motor 1b
'portd.7 - motor 1 enable

'portd.4 - motor 2a
'portc.2 - motor 2b
'portc.0 - motor 2 enable
...
Dim Drift_config As Long                                    'what type of translational drift we're doing
...
Declare Sub Motors_right                                    'both motors on (going right)

Declare Sub Motors_forward                                  'motors forward
Declare Sub Motors_back                                     'motors back
...
Const Min_rpm = 100                                         'minimum RPM for translation / throttling to kick in

Const Max_allowed_rpm = 2250                                'max_rpm allowed before cutting power
Const Max_g = 500                                           'max G's before cutting power

Const Radius = 5                                            'effective radius of circle for accel (centimeters) - seems to be off sometimes...
...
Led_adjust = 15                                             'default setting for heading offset (1 to 100) - if manually set...
Led_adjust_backup = Led_adjust

Load_heading = 0                                            'if set to 0 - will use as heading as set above as opposed to loading from rom

Full_charge_voltage = 70
Base_accel = 489

Const Throttle_percent_max = 100                            'throttle max (for flight - maximizes translation)
...
'   If Accel_raw_data > 522 Then Toggle Portd.5
'   Waitms 40
'Wend
...
'code for driving - only if rudder is pulled left
     If Rudder > Rudder_leftthresh Then


      If Leftright > Rudder_leftthresh Then
         Motors_right
         Waitms 15
      End If

      If Leftright < Rudder_rightthresh Then
          Motors_left
          Waitms 15
      End If
...
'tracking adjustment - if throttle is neither forward or back and throttle is under 50% then go into tracking adjustment mode
   ...
   If Configmode = 1 And Forward = 0 And Backward = 0 And Throttle_percent >= 50 And Load_heading = 1 Then
   ...
   If Configmode = 1 Then Throttle_percent = 40             'ignore throttle if in configmode
...
'   Simpler Code That Just Rounds Up If The Difference Is .5 Or Over
'   If Digitdif => .5 Then Delaytime_long = Delaytime_long + 1
...
'    Low_voltage = 1                                         'kills tail
   ...
   'testing - only does power when moving or spinning up (for flight)
'   Cut_power = 1
'   If Forward = 1 Then Cut_power = 0
'   If Backward = 1 Then Cut_power = 0
'   If Full_power_spin = 1 Then Cut_power = 0
...
Sub Turn_on_tail
         ...
         If Tail_start <= Tail_end Then
            If Led_ref => Tail_start And Led_ref <= Tail_end Then Portd.6 = 1
         Else
            If Led_ref => Tail_start Then Portd.6 = 1
            If Led_ref <= Tail_end Then Portd.6 = 1
         End If
End Sub
...
Sub Motors_off
      Portd.3 = 0
      Portc.1 = 0
      Portd.7 = 0                                           'braking off

      Portd.4 = 0
      Portc.2 = 0
      Portc.0 = 0                                           'braking off
...
Sub Motor1_on
   ...
   If Drift_config = 1 Then
      ...
      Portd.4 = 0
      Portc.2 = 0
      Portc.0 = 0                                           'braking on/off
'      If Forward = 1 Or Backward = 1 Then Portc.0 = 1       'turn on braking if going forward / back

   End If



   If Drift_config = 2 Then
    ...
    If Forward = 1 Or Backward = 1 Then
      If Alternate_motor_cycle = 1 Then
...
Sub Motor2_on
   ...
   If Drift_config = 1 Then
      ...
      Portd.3 = 0
      Portc.1 = 0
      Portd.7 = 0                                           'braking on/off
' If Forward = 1 Or Backward = 1 Then Portd.7 = 1           'turn on braking if going forward / back
   ...
   If Drift_config = 2 Then
    ...
    If Forward = 1 Or Backward = 1 Then
      If Alternate_motor_cycle = 2 Then
