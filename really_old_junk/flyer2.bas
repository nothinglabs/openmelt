...
Dim Pwm_loop As Word

Dim Duty_cycle As Integer
...
Const Min_rpm = 180                                         'minimum RPM for translation / throttling to kick in

Const Radius = 4.05                                         'effective radius of circle for accel (centimeters) - seems to be off sometimes...
...
Declare Sub Motors_pwm_off                                  'turn motor 2 on
...
'for flight
    Begin_brake = Delaytime_long / 9
    Begin_brake = Begin_brake * 8
...
Sub Waitms1
   ...
   Pwm_loop = Pwm_loop + 1
   If Pwm_loop > 20 Then Pwm_loop = 0

   Duty_cycle = 100 - Throttle_percent
   Duty_cycle = Duty_cycle / 5
   Duty_cycle = 20 - Duty_cycle


      If Pwm_loop > Duty_cycle Then
         Motors_pwm_off
      Else
         If Motor_state = 1 Then Motors_left
         If Motor_state = 2 Then Motor1_on
         If Motor_state = 3 Then Motor2_on
      End If
